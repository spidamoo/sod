...
bool animButtonClick(hgeGUIObject* sender) {
	...
	//mainWindow->Show();
	animsWindow->Hide();
	beingInsertedAnimName = ((hgeGUIMenuItem*)sender)->getTitle();
...
bool nextFrameButtonClick(hgeGUIObject* sender) {
	currentFrame += 1;
...
bool prevFrameButtonClick(hgeGUIObject* sender) {
	currentFrame -= 1;
	if (currentFrame < 0) {
		currentFrame = framesCounts[currentMove] - 1;
	}
...
bool nextMoveButtonClick(hgeGUIObject* sender) {
	currentMove += 1;
...
bool prevMoveButtonClick(hgeGUIObject* sender) {
	currentMove -= 1;
	if (currentMove < 0) {
...
bool animDetailsButtonClick(hgeGUIObject* sender) {
	resetMode();
}
...
void loadAnimationsList() {
    ...
    if (loadOkay) {
    	...
    	while (element) {
            ...
            animsWindow->AddCtrl(new hgeGUIMenuItem(101 + i, fnt, 250, 10 + i * 20, listAnimationNames[i], animButtonClick));
	...
	if (mode < MODE_MOVES_INDEX) {
        ...
        for (int i = 0; i < actionsCounts[currentMove]; i++) {
            ...
            for (int j = 0; j < actions[currentMove][i]->getCausesCount(); j++) {
                if (mode == MODE_MOVES_INDEX && x > 700 && x < 950 && y > 100 + i * 75 + j * 12 && y < 112 + i * 75 + j * 12) {
            ...
            for (int j = 0; j < actions[currentMove][i]->getEffectsCount(); j++) {
                ...
                if (mode == MODE_MOVES_INDEX && x > 1050 && x < 1300 && y > 100 + i * 75 + j * 12 && y < 112 + i * 75 + j * 12) {
...
bool RenderFunc() {
    ...
    //			}
        ...
        game->drawRect(625, 425, 675, 475, 0xFF000000, 0);
        for (int index = 0; index < bodiesCount; index++) {
            int j = frameAnimLayer[currentMove][0][index];
            if (frameAnimShow[currentMove][0][j]) {
                animations[j]->RenderEx(
                    650 + frameAnimX[currentMove][0][j] * miniatureScale,
...
//		mainWindow->AddCtrl(new hgeGUIMenuItem(13, fnt, 170, 225, ">>", nextMoveButtonClick));
		...
		animContextMenuWindow->AddCtrl(new hgeGUIMenuItem(51, fnt, 50, 0, "details", animDetailsButtonClick));
		animContextMenuWindow->Hide();
		...
		moveNameLabel = new hgeGUIEditableLabel(game, 221, arial12, 650.0f, 480.0f, 100.0f, 17.0f, "");
...
//		actionCauseContextWindow->AddCtrl(new hgeGUIMenuItem(242, arial12, 187, 200, "add ", addActionCauseClick));
		...
		actionCauseParamInput = new hgeGUIEditableLabel(game, 301, arial12, 62, 5, 120, 17, "0");
		...
		actionEffectParamInput = new hgeGUIEditableLabel(game, 551, arial12, 62, 5, 120, 17, "0");
		actionEffectParamWindowInput->AddCtrl(actionEffectParamInput);
		actionEffectParamInput->setOnChange(actionParamInputChange);
		actionEffectParam2Input = new hgeGUIEditableLabel(game, 552, arial12, 62, 25, 120, 17, "0");
